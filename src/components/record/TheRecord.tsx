        ...
        <Rise>
          <div className="flex flex-wrap items-end justify-between gap-6 pt-14 sm:pt-20">
            <div>
              ...
              <Heading className="mt-4 text-[26px] sm:text-[31px]">The record</Heading>
            </div>
            {latest && (
              <Link
              ...
              >
                ...
                <span className="mt-1.5 block font-display text-[17px] font-light leading-snug text-[#3D3229] transition group-hover:text-[#A38569]">
                  {latest.title}
                </span>
              </Link>
            )}
            ...
            <Rise key={col.heading} delay={0.08 * i} className="border-t border-[#3D3229]/12 lg:border-t-0 lg:border-l lg:border-[#3D3229]/12 lg:px-8 lg:first:border-l-0 lg:first:pl-0 lg:last:pr-0">
              <Link href={col.href} className="group block py-6 lg:py-0">
                <div className="flex items-baseline gap-3">
                  <Heading as="h3" className="text-[21px]">
                    {col.heading}
                  </Heading>
                  <span className="font-mono text-[11px] tabular-nums text-[#A38569]">
                    {col.count} {col.unit}
                  </span>
                </div>
                <p className="mt-3 max-w-[38ch] font-sans text-[13.5px] leading-[1.7] text-[#3D3229]/58">
                  {col.blurb}
                </p>
                <ul className="mt-5 space-y-0">
                  {col.items.map((it, j) => (
                    <li key={it.href}>
                      ...
                      <span className="block truncate py-2 font-sans text-[13px] text-[#3D3229]/62 transition group-hover:text-[#3D3229]">
                        {it.title}
                      </span>
                    </li>
                  ))}
                </ul>
                <span className="mt-4 inline-flex items-baseline gap-2 font-sans text-[12.5px] font-medium text-[#3D3229]">
