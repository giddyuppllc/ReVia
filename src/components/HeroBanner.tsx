...
const FINE_PRINT =
  "A trusted laboratory supply source for independently tested, research-grade peptides. Every batch verified to the >98% purity specification by RP-HPLC. For research use only.";

// Bottom bar is facts only now.
const INFO = [
  { icon: FlaskConical, text: `${COA_SPEC.puritySpec} Purity` },
  { icon: Atom, text: "RP-HPLC Verified" },
  { icon: ShieldCheck, text: "US-Manufactured · cGMP Certified" },
  { icon: Truck, text: "Per-Batch COA" },
  ...
  return (
    <section className="relative h-[58vh] sm:h-[74vh]">
      ...
      <div className="pointer-events-none absolute inset-0 z-30 flex items-center justify-center px-5">
        ...
        >
          {/* Shop — leaves for the partner storefront; this site does not sell.
              NOTE: globals.css remaps the whole sky and blue scales to warm
              mocha (--color-sky-500: #A38569), so `bg-sky-500` renders brown.
              An actual blue has to be an explicit hex — this one matches the
              logo swoosh and the vial label bands. */}
          <a
            href={d2cUrl()}
            {...(D2C.isLive ? PARTNER_LINK_PROPS : {})}
            title={D2C.isLive ? `Opens ${D2C.name} in a new tab` : undefined}
          ...
          {/* What's a Stack? — glass, to match the bars */}
          <Link
            href="/stacks"
